    ...
    <div className="min-h-screen bg-slate-50">
      <main className="max-w-[1600px] mx-auto p-4 md:p-6 xl:p-8 space-y-6">
        <div>
          ...
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Overspent Projects</h2>
          <p className="text-sm text-slate-500 mt-0.5">Projects where actual spending exceeds budget</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-16 space-y-2">
              <p className="text-emerald-600 text-5xl">✓</p>
              <p className="text-slate-500 font-medium">All projects are within budget – great job!</p>
            </div>
          ) : (
              ...
              <table className="w-full min-w-[600px]">
                <thead>
                  <tr className="border-b border-slate-100">
                    {["Project","Budget","Actual","Overspent","Util %"].map(h => (
                ...
                <tbody>
                  {rows.map((r, i) => (
                    <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="py-3 px-5 text-sm font-bold text-red-600 tabular-nums">{formatPKR(r.actual)}</td>
                      <td className="py-3 px-5 text-sm font-bold text-red-600 tabular-nums">{formatPKR(r.over)}</td>
                      <td className="py-3 px-5 text-sm text-red-600 tabular-nums">{r.pct}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
