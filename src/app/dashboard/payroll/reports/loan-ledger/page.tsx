...
export default function LoanLedgerPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      if (cid) {
        ...
        // Fetch all loans for trial balance
        supabase
          .from("employee_loans")
          .select("id, employee_id, principal_amount, balance, status, employees!inner(full_name, employee_code)")
  ...
  const fetchLedger = async () => {
    ...
    // Get deduction lines for this employee from payroll runs
    const { data: lines } = await supabase
      ...
      .eq("source_type", "loan")
      .eq("source_id", selectedEmployee)   // note: source_id is loan id, not employee id – need to filter by employee
      .order("created_at", { ascending: true })

    // Actually we need to filter by employee via loan -> employee. Let's do a simpler approach:
    // Fetch all loan deductions for any loan of this employee.
    const { data: loansForEmp } = await supabase
      .from("employee_loans")
      .select("id")
      .eq("employee_id", selectedEmployee)

    const loanIds = (loansForEmp || []).map(l => l.id)
    if (loanIds.length === 0) {
      setLedgerLines([])
      setLoading(false)
      return
  ...
  const downloadPDF = () => {
    const doc = new jsPDF()
    doc.setFontSize(14)
    doc.text("Loan Ledger & Trial Balance", 14, 20)
    doc.setFontSize(10)
    ...
    // Trial balance
    const trialRows = loans.map(loan => {
      const emp = Array.isArray(loan.employees) ? loan.employees[0] : loan.employees
      return [emp?.employee_code || "N/A", emp?.full_name || "Unknown", loan.principal_amount.toLocaleString(), loan.balance.toLocaleString(), loan.status]
    })
    const finalY = (doc as any).lastAutoTable.finalY + 15
    doc.setFontSize(12)
    doc.text("Loan Trial Balance", 14, finalY)
    ;(doc as any).autoTable({
      startY: finalY + 6,
      head: [["Code", "Employee", "Principal", "Balance", "Status"]],
      body: trialRows,
    })
    doc.save(`Loan_Ledger_${selectedEmployee}.pdf`)
  }
  ...
  return (
    <div style={{ padding: 24, background: "var(--bg)", minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: "var(--text)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)", margin: 0 }}>💵 Loan Ledger & Trial Balance</h1>
      </div>
      ...
      <div className="card">
        <h3>Loan Trial Balance</h3>
        <table className="table">
          <thead>
            <tr><th>Code</th><th>Employee</th><th>Principal</th><th>Balance</th><th>Status</th></tr>
          </thead>
          <tbody>
            {loans.map((loan) => {
              const emp = Array.isArray(loan.employees) ? loan.employees[0] : loan.employees
        ...
        <div style={{ marginTop: 12, fontWeight: 700 }}>
          <div>Total Principal: PKR {totalPrincipal.toLocaleString()}</div>
