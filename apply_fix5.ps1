...
//
// ===================== PUT - Edit an existing contribution =====================
//
// Calls update_investor_contribution RPC, which reverses the old journal
// entry, posts a new one for the corrected amount, updates the
// investor_contributions row (status='edited'), and recomputes
// capital_contributed fresh from source rows. Original entry is never
// deleted - full audit trail preserved, matching update_receipt_transaction.
//
...
export async function PUT(request: NextRequest) {
  ...
  const body = await request.json()
  const { contribution_id, amount, contribution_date, bank_account_id, reference, notes } = body

  if (!contribution_id || !amount || amount <= 0) {
    return NextResponse.json({ error: 'contribution_id and a positive amount are required' }, { status: 400 })
  }
  if (!bank_account_id) {
    return NextResponse.json({ error: 'bank_account_id is required' }, { status: 400 })
  }
  if (!contribution_date) {
    return NextResponse.json({ error: 'contribution_date is required' }, { status: 400 })
  }
  ...
  const { data, error: rpcError } = await supabase.rpc('update_investor_contribution', {
    p_contribution_id: contribution_id,
    p_company_id: companyId,
    p_amount: amount,
    p_contribution_date: contribution_date,
  ...
  if (rpcError || !data?.success) {
    return NextResponse.json({ error: rpcError?.message || data?.error || 'Failed to update contribution' }, { status: 500 })
  }
