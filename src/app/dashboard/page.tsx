...
function Sparkline({ values, color }: { values: number[]; color: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ width: "100%", height: 32, display: "block", marginTop: 4 }}>
      ...
      <polygon points={area} fill={`url(#${uid})`} /><polyline points={line} fill="none" stroke={color} strokeWidth="1.5" strokeLinejoin="round" strokeLinecap="round" />
    </svg>
  )
}
      ...
      {values.map((v, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", height: "100%", justifyContent: hasNeg ? "center" : "flex-end" }}>
            {neg ? (<><div style={{ flex: 1 }} /><div style={{ width: "100%", height: barH, background: "#EF4444", borderRadius: "0 0 3px 3px", opacity: .8 }} /><div style={{ width: "100%", height: 1, background: "#E2E8F0" }} /></>)
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 8, marginTop: 4 }}><div style={{ width: 3, height: 14, background: "#1E3A8A", borderRadius: 2, flexShrink: 0 }} /><span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#1E3A8A" }}>{children}</span></div>
}

function KpiCard({ label, value, subtitle, accent, icon, isCurrency = true, trend, href }: { label: string; value: number; subtitle: string; accent: string; icon: React.ReactNode; isCurrency?: boolean; trend?: number[]; href?: string }) {
  const display = isCurrency ? `PKR ${SHORT(value)}` : value.toLocaleString()
  return (
    <div style={{ background: "white", borderRadius: 10, border: "1px solid #E2E8F0", borderTop: `3px solid ${accent}`, display: "flex", flexDirection: "column", transition: "box-shadow .15s,transform .15s", cursor: href ? "pointer" : "default", overflow: "hidden", height: "100%" }}
      ...
      <div style={{ marginLeft: 0, marginRight: 0, padding: "5px 13px", background: "#F8FAFC", borderTop: "1px solid #E2E8F0", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "clamp(9px,0.7vw,11px)", fontWeight: 600, color: href ? "#1E3A8A" : "#475569" }}>
...
function ChartCard({ title, badge, badgeColor, labels, values, color }: { title: string; badge: string; badgeColor: string; labels: string[]; values: number[]; color: string }) {
  return (
    <div style={{ background: "white", borderRadius: 10, border: "1px solid #E2E8F0", padding: "11px 13px 10px" }}>
      ...
      {labels.length ? <BarChart labels={labels} values={values} color={color} /> : <div style={{ height: 80, display: "flex", alignItems: "center", justifyContent: "center", color: "#CBD5E1", fontSize: 12 }}>No data yet</div>}
    </div>
  )
}
...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: overdueData } = await supabase.from("invoices").select("id,invoice_no,total,paid,due_date,customers!party_id(name,phone)").eq("type", "sale").lt("due_date", today).neq("status", "Paid").limit(5)
      ...
      setIncomeChart({ labels: months, values: revValues }); setProfitChart({ labels: months, values: profitValues })
  ...
  if (loading) return (
    <div style={{ padding: 24, background: "#EFF4FB", minHeight: "100vh", fontFamily: "Arial" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 10, marginBottom: 16 }}>
        {[...Array(8)].map((_, i) => (
          <div key={i} style={{ height: 110, background: "#F1F5F9", borderRadius: 10, animation: "pulse 1.5s ease-in-out infinite", animationDelay: `${i * 80}ms` }} />
        ))}
      </div>
      <div style={{ height: 140, background: "#F1F5F9", borderRadius: 10 }} />
    </div>
  )
  ...
  return (
    <div style={{ padding: "clamp(8px,1.5vw,16px) clamp(12px,2.5vw,24px) clamp(16px,2.5vw,24px)", background: "#EFF4FB", minHeight: "100%", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
        @keyframes pulse { 0%,100% { opacity: 1 } 50% { opacity: .45 } }
        @keyframes spin { to { transform: rotate(360deg) } }
        .kpi-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(min(100%, 160px), 1fr)); gap: clamp(7px, 1.2vw, 12px); margin-bottom: clamp(10px, 1.8vh, 16px); }
        .chart-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(min(100%, 280px), 1fr)); gap: clamp(7px, 1.2vw, 12px); margin-bottom: clamp(10px, 1.8vh, 16px); }
        ...
        @media (max-width: 900px) { .ov-header { grid-template-columns: 1fr 110px 100px; } .ov-row { grid-template-columns: 1fr 110px 100px; } .ov-col-bal { display: none !important; } .ov-bal-inline { display: block !important; } }
        @media (max-width: 600px) { .ov-header { display: none !important; } .ov-row { grid-template-columns: 1fr 1fr !important; grid-template-rows: auto auto; gap: 4px; padding: 10px 14px; } }
        .ov-row:hover { background: #FAFBFF; }
        .ov-bal-inline { display: none; }
      `}</style>

      <div className="dash-inner">
        {!online && <div style={{ background: "#FEF2F2", border: "1px solid #FECACA", color: "#B91C1C", padding: "8px 14px", borderRadius: 8, marginBottom: 10, display: "flex", alignItems: "center", gap: 8, fontSize: "clamp(11px,0.9vw,13px)", fontWeight: 500 }}><WifiOff size={14} /> You are offline</div>}

        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8 }}>
          <button onClick={fetchData} disabled={refreshing}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", background: "white", border: "1px solid #E2E8F0", borderRadius: 7, fontSize: "clamp(10px,0.8vw,11.5px)", fontWeight: 600, color: "#475569", cursor: "pointer", fontFamily: "inherit" }}>
        ...
        <div className="kpi-grid">
          ...
          <KpiCard label="Payables" value={ops.payables} subtitle="Outstanding supplier bills" accent="#EF4444" icon={<TrendingDown size={13} />} trend={incomeChart.values.map(v => v * 0.15)} href="/dashboard/reports/ap-aging" />
        ...
        <div className="chart-grid">
          ...
          <ChartCard title="Monthly Profit" badge="Last 6 months" badgeColor="#10B981" labels={profitChart.labels} values={profitChart.values} color={profitChart.values.length > 0 && profitChart.values.every(v => v >= 0) ? "#10B981" : "#1D4ED8"} />
        </div>

        <SectionLabel>Overdue Invoice Reminders</SectionLabel>
        <div style={{ background: "white", borderRadius: 10, border: "1px solid #E2E8F0", overflow: "hidden", marginBottom: 10 }}>
          {overdue.length === 0 ? (
            <div style={{ padding: "16px 18px", display: "flex", alignItems: "center", gap: 8, color: "#10B981", fontSize: "clamp(12px,1vw,13.5px)", fontWeight: 500 }}><CheckCircle2 size={16} /> No overdue invoices — all clear!</div>
          ) : (
            <>
              <div className="ov-header"><span>Invoice / Customer</span><span>Due Date</span><span className="ov-col-bal" style={{ textAlign: "right" }}>Balance</span><span style={{ textAlign: "right" }}>Action</span></div>
              {overdue.map((inv, i) => (
                <div key={inv.id} className="ov-row">
                  <div style={{ minWidth: 0 }}><span style={{ fontWeight: 700, color: "#1E293B" }}>{inv.invoice_no}</span><span style={{ color: "#64748B", marginLeft: 6, fontSize: "clamp(10px,0.8vw,12px)" }}>{inv.customer_name}</span></div>
                  <div style={{ display: "flex", alignItems: "center", gap: 4, color: "#EF4444", fontWeight: 600, fontSize: "clamp(9.5px,0.78vw,11px)" }}><AlertTriangle size={11} /> {inv.due_date}</div>
                  <div className="ov-col-bal" style={{ textAlign: "right", fontWeight: 700, color: "#1E293B", fontSize: "clamp(10px,0.82vw,12px)" }}>{PKR(inv.balance)}</div>
                  <div style={{ textAlign: "right" }}>
                    {inv.customer_phone ? <a href={waLink(inv.customer_phone, inv.invoice_no, inv.balance, inv.customer_name)} target="_blank" rel="noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "#25D366", color: "white", padding: "4px clamp(7px,1vw,10px)", borderRadius: 6, fontSize: "clamp(9.5px,0.78vw,11px)", fontWeight: 600, textDecoration: "none", whiteSpace: "nowrap" }}><MessageCircle size={11} /> Remind</a> : <span style={{ fontSize: "clamp(9px,0.72vw,10.5px)", color: "#CBD5E1" }}>No phone</span>}
                  </div>
                </div>
              ))}
            </>
        ...
        <div style={{ background: "white", borderRadius: 8, border: "1px solid #E2E8F0", padding: "8px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 7, height: 7, borderRadius: "50%", background: profitable ? "#10B981" : "#EF4444", boxShadow: `0 0 0 3px ${profitable ? "#10B98133" : "#EF444433"}` }} /><span style={{ fontSize: "clamp(10px,0.8vw,12px)", color: "#64748B" }}>Business Health: <strong style={{ color: profitable ? "#10B981" : "#EF4444" }}>{profitable ? "Profitable" : "Loss-Making"}</strong></span></div>
          <div style={{ display: "flex", gap: "clamp(10px,1.5vw,18px)", flexWrap: "wrap" }}>
            {[["Revenue", PKR(kpis.revenue)], ["Expenses", PKR(kpis.expenses)], ["Products", String(ops.total_products)], ["Customers", String(ops.total_customers)]].map(([k, v]) => <span key={k} style={{ fontSize: "clamp(9px,0.75vw,11px)", color: "#64748B" }}>{k}: <strong style={{ color: "#1E293B" }}>{v}</strong></span>)}
          </div>
        </div>
      </div>
    </div>
