...
export default function ReceiptViewerClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    fetch(`/api/public/receipt?id=${id}`)
      .then(r => r.json())
      .then(d => {
      ...
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        <div style={{ background: "white", borderRadius: 16, padding: "28px 32px", marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
            ...
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em" }}>Receipt</div>
              <div style={{ fontSize: 24, fontWeight: 800, color: "#1740c8", marginTop: 4 }}>{receipt.receipt_no}</div>
            </div>
          </div>
        </div>

        <div style={{ background: "white", borderRadius: 16, padding: "20px 22px", marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
          <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: "#94a3b8", letterSpacing: "0.06em", marginBottom: 10 }}>Received From</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#0f172a", marginBottom: 4 }}>{receipt.customer_name}</div>
          {receipt.customer_address && <div style={{ fontSize: 12, color: "#64748b", marginBottom: 2 }}>{receipt.customer_address}</div>}
          {receipt.customer_phone && <div style={{ fontSize: 12, color: "#64748b" }}>Phone: {receipt.customer_phone}</div>}
        </div>

        <div style={{ background: "white", borderRadius: 16, padding: "20px 22px", marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
          <DetailRow label="Date" value={receipt.date} />
          {receipt.bank_name && <DetailRow label="Bank" value={receipt.bank_name} />}
          {receipt.reference && <DetailRow label="Reference" value={receipt.reference} />}
        ...
        {receipt.notes && (
          <div style={{ background: "white", borderRadius: 16, padding: "20px 22px", marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
            ...
            <div style={{ fontSize: 13, color: "#475569", lineHeight: 1.6 }}>{receipt.notes}</div>
          </div>
        )}
