/**
 * receiptPDF.ts
 * Generates a payment receipt PDF using the same premium format as the invoice.
 */
...
export async function generateReceiptPDF(data: ReceiptPDFData): Promise<jsPDF> {
  ...
  const isPaid = statusText === "PAID" || statusText === "ACTIVE"
  const badgeColor: [number,number,number] = isPaid ? [5,150,105] : RED
  ...
  if (termsLines.length === 0) termsLines.push("Thank you for your payment.")
