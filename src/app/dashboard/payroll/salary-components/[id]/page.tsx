      ...
      if (cid) {
        ...
        supabase.from("salary_components").select("*").eq("id", componentId).eq("company_id", cid).single().then(({ data }) => {
          if (data) {
            ...
            setGlAccountId(data.gl_account_id)
  ...
  return (
    <div style={{ padding: 24, background: "var(--bg)", minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: "var(--text)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1 }}>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)", margin: 0 }}>✏️ Edit Salary Component</h1>
          <p style={{ color: "var(--text-muted)", fontSize: 13 }}>Modify component details</p>
        </div>
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
        <div className="card">
          ...
          <button className="btn btn-submit" type="submit" disabled={loading}>
            <Save size={16} /> {loading ? "Saving..." : "Update Component"}
          </button>
        </div>
      </form>
    </div>
