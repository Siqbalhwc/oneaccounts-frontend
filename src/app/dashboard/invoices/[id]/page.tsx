...
export default function InvoiceDetailPage() {
        ...
        if (items && items.length > 0) {
          ...
          if (productIds.length > 0) {
            ...
            inv.items = items.map((item: any) => {
              ...
              return { ...item, product_code: prod?.code || "", product_name: prod?.name || "", product_image: prod?.image_path || null }
            })
          } else {
            inv.items = items.map((item: any) => ({ ...item, product_code: "", product_name: "", product_image: null }))
          }
        } else {
          inv.items = []
        }
      ...
      .then(({ data: lines }) => {
        if (lines && lines.length > 0) {
          setJournalLines(lines.map((l: any) => ({
            account_id: l.account_id, account_code: l.accounts?.code || "", account_name: l.accounts?.name || "",
  ...
  const handlePrintPDF = async () => {
    ...
    const pdfData = {
      companyName: companyName || "", companyAddress: "", companyPhone: "", companyEmail: "",
      companyTagline: companyTagline || "", logoUrl, businessType: "",
      invoiceNo: invoice.invoice_no, date: invoice.date, dueDate: invoice.due_date,
      customerName: customer?.name || "Unknown", customerAddress: customer?.address || "",
      customerPhone: customer?.phone || "", customerEmail: customer?.email || "",
      paymentTerms: customer?.payment_terms || null, createdBy: invoice.created_by || "—", status: invoice.status,
      items: (invoice.items || []).map(item => ({
        description: item.description || "", qty: item.qty || 0, unit_price: item.unit_price || 0,
        total: item.total || 0, image_path: item.product_image || null,
        product_id: item.product_code || null, product_name: item.product_name || "",
    ...
    <div style={{ padding: 24, background: "var(--bg)", minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: "var(--text)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button className="btn" onClick={() => router.push("/dashboard/invoices")}><ArrowLeft size={16} /></button>
          <div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button className="btn btn-primary" onClick={handlePrintPDF}><Printer size={14} /> Print PDF</button>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid-2col">
          ...
          <div>
            ...
            <div className="value">{invoice.due_date}</div>
          </div>
          <div>
            <div className="label">Customer</div>
            <div className="value">{invoice.customer?.code} – {invoice.customer?.name}</div>
          </div>
          <div>
                ...
                {invoice.items.map(item => (
                  <tr key={item.id}>
                    <td style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      {item.product_image ? <img src={item.product_image} alt="" style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 4 }} /> : <div style={{ width: 28, height: 28, background: "var(--card-hover)", borderRadius: 4 }} />}
