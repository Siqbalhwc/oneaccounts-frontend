...
function DashboardLayoutInner({
  ...
  // KEY FIX: if role hasn't loaded yet, skip role-filtering entirely
  // so items are never hidden due to a null/undefined role mid-load
  const isVisible = (leaf: NavLeaf): boolean => {
    if (leaf.feature !== null && !hasFeature(leaf.feature)) return false
    if (role !== null && role !== undefined && role !== "" && leaf.roles.length > 0 && !leaf.roles.includes(role)) return false
    return true
  }
  ...
  // ── hrefs claimed by non-SYSTEM sections ───────────────────────────────────
  const claimedHrefs = new Set([
    ...
    "/dashboard/accounts", "/dashboard/journal",
    "/dashboard/reports",
  ...
  const navSections: NavSection[] = [
    ...
    },
    {
      ...
      items: allNavItems.filter(i =>
        ["/dashboard/customers","/dashboard/invoices","/dashboard/receipts",
         "/dashboard/suppliers","/dashboard/bills","/dashboard/payments"].includes(i.href)
      ),
    },
    {
      section: "BANKING",
    ...
    },
    {
      ...
      groups: [
        {
          groupLabel: "General",
          items: allNavItems.filter(i => ["/dashboard/accounts","/dashboard/journal"].includes(i.href)),
        },
        {
          groupLabel: "Reports",
          items: allNavItems.filter(i => ["/dashboard/reports"].includes(i.href)),
        },
        {
  ...
  return (
    <>
      <style>{`
        ...
        .dl-section-content {
          margin-left: 6px; border-left: 1px solid rgba(255,255,255,0.08);
        ...
        .dl-nav-item {
          ...
          text-decoration: none; transition: all 0.15s; margin-bottom: 2px; cursor: pointer;
        }
        ...
        .dl-nav-group-label {
          ...
          color: rgba(255,255,255,0.25); padding: 4px 10px 2px; letter-spacing: 0.05em;
        }
        .dl-sidebar-user {
        ...
        @media (max-width: 768px) {
          ...
          .dl-section-btn span:last-child, .dl-nav-group-label,
        ...
        <aside className="dl-sidebar" id="dl-sidebar">
          ...
          <nav className="dl-sidebar-nav">
            {navSections.map((sec) => {
              ...
              // CRITICAL: never skip a section just because items are empty
              // role may still be loading — always render the section header
              // and show items once role resolves
