...
export default function RemainingFundsPage() {
  ...
  useEffect(() => {
    ...
    supabase.rpc("dashboard_project_utilization", {
      ...
      })).filter((p: any) => selectedProjectId ? p.project === projects.find(pr => pr.id == selectedProjectId)?.name : true)
      setRows(enriched)
      setLoading(false)
    })
      ...
      <div className="card" style={{ overflowX: "auto" }}>
        {loading ? <p>Loading...</p> : rows.length === 0 ? <p style={{ color: "#94a3b8", textAlign: "center", padding: 40 }}>No data found.</p> : (
          <table className="table" style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr><th>Project</th><th>Budget</th><th>Actual</th><th>Remaining</th><th>Util %</th></tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
                <tr key={i}>
                  ...
                  <td style={{ fontWeight: 600, color: r.remaining < 0 ? "#dc2626" : "#16a34a" }}>{r.remaining?.toLocaleString()}</td>
                  <td>{r.pct}%</td>
                </tr>
              ))}
            </tbody>
