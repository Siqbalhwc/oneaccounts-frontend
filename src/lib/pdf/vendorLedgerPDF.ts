/**
 * vendorLedgerPDF.ts
 * Generates a professional Vendor Ledger report (landscape).
 */
...
export interface VendorLedgerPDFData {
  companyName:    string
...
export async function generateVendorLedgerPDF(data: VendorLedgerPDFData): Promise<jsPDF> {
  ...
  doc.text("VENDOR LEDGER", PW - MR, LOGO_Y + 8, { align: "right" })

  const metaY = LOGO_Y + 18
  doc.setFont("helvetica", "normal").setFontSize(8).setTextColor(...MUTED)
  doc.text(`Supplier: ${data.supplierCode} – ${data.supplierName}`, PW - MR, metaY, { align: "right" })
  ...
  // Column widths (same as customer ledger)
  const dateW = 28
  const entryW = 41
  const debitW = 30
  const creditW = 30
