      ...
      <div style={{ marginBottom: 16 }}>
        ...
        {items.map((a: any) => (
          <div key={a.id} className="acc-row" onClick={() => openLedger(a.id)}>
            <span style={{ fontSize: 10, color: "#94A3B8", minWidth: 40 }}>{a.code}</span>
            <span style={{ fontSize: 12, color: "#E2E8F0", flex: 1, paddingLeft: 10 }}>{a.name}</span>
      ...
      <div className="bs-header">
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#94A3B8" }}>Shahid Iqbal &amp; Co · Assets = Liabilities + Equity · Click to drill down</div>
          </div>
        </div>
      ...
      <div className="bs-body">
        ...
        <div className="bs-col">
          ...
          <div className="subtotal-band" style={{ color: "#F87171" }}>
            ...
            <span>PKR {fmt(totalLiabilities)}</span>
          </div>
          ...
          {(grouped["Equity"] || []).map((a: any) => (
            <div key={a.id} className="acc-row" onClick={() => openLedger(a.id)}>
              <span style={{ fontSize: 10, color: "#94A3B8", minWidth: 40 }}>{a.code}</span>
              <span style={{ fontSize: 12, color: "#E2E8F0", flex: 1, paddingLeft: 10 }}>{a.name}</span>
              <span style={{ fontSize: 12, color: "#CBD5E1" }}>PKR {fmt(a.balance || 0)}</span>
            </div>
          ))}
          {/* Retained Earnings */}
          ...
          <div className="subtotal-band" style={{ color: "#C4B5FD", marginTop: 4 }}>
            ...
            <span>{sign(totalEquity)}PKR {fmt(totalEquity)}</span>
          </div>
