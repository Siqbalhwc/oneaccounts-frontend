...
export default function ManagementDashboard({ role }: { role: string }) {
  ...
  const Trend = ({ value, positive = false, negative = false }: { value: number; positive?: boolean; negative?: boolean }) => {
    ...
    if (value > 0) return <span style={{ display: "flex", alignItems: "center", gap: 2, color: positive ? "var(--kpi-positive)" : "var(--kpi-warn)", fontSize: "0.75rem", fontWeight: 600 }}>
      <TrendingUp size={14} /> {Math.abs(value)}%
    </span>
    if (value < 0) return <span style={{ display: "flex", alignItems: "center", gap: 2, color: negative ? "var(--kpi-warn)" : "var(--kpi-positive)", fontSize: "0.75rem", fontWeight: 600 }}>
  ...
  if (companyError) {
    return (
      <div style={{ padding: 40, textAlign: "center", background: "var(--bg)", minHeight: "100vh", color: "var(--text-muted)" }}>
        <div style={{ fontSize: "1.2rem", marginBottom: 8, color: "var(--kpi-negative)" }}>Could not load dashboard</div>
    ...
    return (
      <div style={{ padding: 40, textAlign: "center", background: "var(--bg)", minHeight: "100vh", color: "var(--text-muted)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
        <motion.div
          ...
          style={{ width: 36, height: 36, borderRadius: "50%", border: "3px solid var(--border)", borderTop: "3px solid var(--kpi-info)" }}
        />
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100%", flex: 1, fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif", color: "var(--text)", transition: "background 0.3s, color 0.3s" }}>
      <style>{`
        ...
        .progress-fill { height: 100%; border-radius: 10px; background: var(--kpi-positive); }
          ...
          <motion.div className="card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.5 }}>
            ...
            {projectRows.length > 5 && (
              <div style={{ textAlign: "right", marginTop: "0.5rem" }}>
                <button className="warning-btn" style={{ background: "transparent", color: "var(--kpi-link)", border: "1px solid #334155", padding: "4px 12px" }} onClick={() => router.push("/dashboard/settings/budgets" + detailQuery())}>View All Projects →</button>
              </div>
            )}
          </motion.div>
            ...
            {donorBalances.map((d: any, idx: number) => (
              <div key={idx} onClick={() => router.push(`/dashboard/settings/budgets?donor=${d.donor_id}&fy=${fiscalYear}`)} style={{ display: "flex", alignItems: "center", gap: "0.8rem", background: "var(--card)", borderRadius: "12px", padding: "0.5rem 1rem", border: "1px solid var(--border)", cursor: "pointer", marginBottom: "0.5rem", flexWrap: "wrap" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "0.65rem", color: "var(--text-muted)" }}>{d.monthsPassed}/{d.monthsTotal} months · {d.health === "slow" ? <span style={{ color: "var(--kpi-negative)", fontWeight: 600 }}>Slow: only {d.pct}% spent</span> : d.health === "ok" ? <span style={{ color: "var(--kpi-warn)", fontWeight: 600 }}>OK</span> : <span style={{ color: "var(--kpi-positive)", fontWeight: 600 }}>On Track</span>}</div>
                </div>
                ...
                {underspentActivities.map((act: any, idx: number) => (
                  <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 80px 80px 100px", gap: 8, alignItems: "center", padding: "5px 0", borderBottom: "1px solid var(--border)", fontSize: "0.8rem" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                      <span style={{ fontWeight: 600, color: "var(--kpi-positive)", fontSize: "0.75rem" }}>{formatPKR(act.remaining)}</span>
                      <span style={{ fontSize: "0.7rem", color: "var(--kpi-positive)", whiteSpace: "nowrap" }}>({act.unspentPct}%)</span>
                    </div>
                  </div>
                ))}
                <div style={{ textAlign: "right", marginTop: "0.5rem" }}>
                  <button className="warning-btn" style={{ background: "transparent", color: "var(--kpi-link)", border: "1px solid #334155", padding: "4px 12px" }} onClick={() => router.push("/dashboard/reports/budget-vs-actual" + detailQuery())}>View all →</button>
                </div>
              </>
            )}
          </motion.div>
