...
export default function SupplierLedgerPage() {
  ...
  const fetchLedger = async () => {
    ...
    try {
      ...
      if (!lines) { setLedgerLines([]); setLoading(false); return }
  ...
  useEffect(() => {
    if (supplierId && companyId) fetchLedger()
  }, [supplierId, companyId, startDate, endDate])
  ...
  if (!supplierId) return <div style={{ padding: 24, textAlign: "center", color: "var(--text-muted)" }}>No supplier selected.</div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)", margin: 0 }}>
            🚚 Supplier Ledger: {supplier ? `${supplier.code} – ${supplier.name}` : "Loading..."}
          </h1>
          <p style={{ color: "var(--text-muted)", fontSize: 13, margin: 0 }}>Transaction history for this supplier</p>
        </div>
