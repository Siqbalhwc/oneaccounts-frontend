...
export default function NewBillPage() {
  ...
  const [activities, setActivities] = useState<any[]>([])  // all activities, not filtered by location initially
  ...
  const selectSupplier = (s: any) => {
    ...
    // If the supplier has default location/activity, we could pre-fill the first item, but we won't force it.
  }
  ...
  const fetchProjectAndDonor = async (activityId: number) => {
    if (projectCache[activityId] && donorCache[activityId]) return // already cached
    try {
  ...
  // ── Item management (per‑line tags restored) ──
  const addItem = () => {
  ...
  const handleSubmit = async () => {
    ...
    const enrichedItems = items.map(item => ({
      ...
      project_id: projectCache[item.activity_id]?.id ?? null,
      donor_id: donorCache[item.activity_id]?.id ?? null,
    }))
      ...
      <div className="inv-shell">
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:12, color:"#94A3B8", marginTop:1 }}>Select supplier, then add items with individual location, activity & GL</div>
          </div>
