...
export default function RemainingFundsPage() {
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, { head: [["Project","Budget","Actual","Remaining","Util %"]], body, startY: 35 })
  ...
  return (
    <div style={{ padding: "20px 24px", fontFamily: "Segoe UI, system-ui, sans-serif", background: "#f0f4f8", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
        ...
        <div>
          <h2 style={{ fontSize: 22, fontWeight: 800, color: "#0f172a", margin: 0 }}>Remaining Funds</h2>
          <p style={{ fontSize: 14, color: "#64748b", margin: 0 }}>Budget vs Actual with remaining balance per project</p>
        </div>
      </div>
      ...
      <div className="card" style={{ overflowX: "auto" }}>
        {loading ? <p>Loading...</p> : rows.length === 0 ? <p style={{ color: "#94a3b8", textAlign: "center", padding: 40 }}>No data found.</p> : (
          <table className="table" style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th>Project</th><th>Budget</th><th>Actual</th><th>Remaining</th><th>Util %</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
                <tr key={i}>
                  ...
                  <td style={{ textAlign: "right", fontWeight: 600 }}>{formatPKR(r.actual)}</td>
                  <td style={{ textAlign: "right", fontWeight: 600, color: r.remaining < 0 ? "#dc2626" : "#16a34a" }}>{formatPKR(r.remaining)}</td>
