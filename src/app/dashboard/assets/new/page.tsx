...
export default function NewAssetPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      if (!cid) return
      setCompanyId(cid)
      supabase.from("accounts").select("id,code,name,type").eq("company_id", cid).in("type", ["Asset","Expense"]).order("code").then(r => r.data && setAccounts(r.data))
      supabase.from("locations").select("id,name").eq("company_id", cid).order("name").then(r => r.data && setLocations(r.data))
      supabase.from("personnel").select("id,name").eq("company_id", cid).is("deleted_at", null).order("name").then(r => r.data && setPersonnel(r.data))
    })
  }, [])

  const handleSubmit = async () => {
    if (!name || !purchaseDate || !costPrice || !lifeMonths) { setError("Please fill all required fields."); return }
    setSaving(true); setError(""); setFlash("")
    const res = await fetch("/api/assets", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name, category, purchase_date: purchaseDate, cost_price: parseFloat(costPrice), life_months: parseInt(lifeMonths),
        salvage_value: parseFloat(salvageValue) || 0, location_id: locationId || null,
        responsible_person_id: personId || null, gl_asset_account_id: assetAcctId || null,
        gl_accum_dep_account_id: accumDepAcctId || null, gl_dep_expense_account_id: depExpAcctId || null,
        notes, source_type: "manual",
      }),
    })
    const result = await res.json()
    if (!result.success) setError(result.error || "Failed to save asset")
    else { setFlash(`✅ Asset ${result.asset.asset_no} created!`); setTimeout(() => router.push(`/dashboard/assets/${result.asset.id}`), 800) }
    setSaving(false)
  }
  ...
  const monthlyDep = Math.round((cost - salvage) / life)
      ...
      <div className="header-grid">
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div className="card">
            ...
            <div className="row">
              <div><label className="label">Category</label><input className="input" value={category} onChange={e => setCategory(e.target.value)} placeholder="e.g. Furniture" /></div>
              <div><label className="label">Purchase Date *</label><input className="input" type="date" value={purchaseDate} onChange={e => setPurchaseDate(e.target.value)} /></div>
            </div>
            <div className="row">
              <div><label className="label">Cost Price *</label><input className="input" type="number" min="0" step="100" value={costPrice} onChange={e => setCostPrice(e.target.value)} placeholder="0" /></div>
              <div><label className="label">Life (Months) *</label><input className="input" type="number" min="1" value={lifeMonths} onChange={e => setLifeMonths(e.target.value)} /></div>
            </div>
            <div className="row">
            ...
            <div style={{ marginBottom:12 }}>
              ...
              <select className="select" value={personId} onChange={e => setPersonId(e.target.value)}><option value="">— Select —</option>{personnel.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
            </div>
            <div style={{ marginBottom:12 }}>
              <label className="label">GL Asset Account</label>
              <select className="select" value={assetAcctId} onChange={e => setAssetAcctId(e.target.value)}><option value="">— Select —</option>{accounts.filter(a => a.type === "Asset").map(a => <option key={a.id} value={a.id}>{a.code} – {a.name}</option>)}</select>
            </div>
            <div className="row">
              <div><label className="label">Accum. Dep. Account</label><select className="select" value={accumDepAcctId} onChange={e => setAccumDepAcctId(e.target.value)}><option value="">— Select —</option>{accounts.filter(a => a.type === "Asset").map(a => <option key={a.id} value={a.id}>{a.code} – {a.name}</option>)}</select></div>
              <div><label className="label">Dep. Expense Account</label><select className="select" value={depExpAcctId} onChange={e => setDepExpAcctId(e.target.value)}><option value="">— Select —</option>{accounts.filter(a => a.type === "Expense").map(a => <option key={a.id} value={a.id}>{a.code} – {a.name}</option>)}</select></div>
            </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12, position:"sticky", top:16 }}>
          <div className="card">
            ...
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, marginBottom:4 }}>
              ...
              <span style={{ fontWeight:600 }}>PKR {cost.toLocaleString()}</span>
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, marginBottom:4 }}>
              <span style={{ color:"var(--text-muted)" }}>Life</span>
              <span style={{ fontWeight:600 }}>{life} months</span>
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:13 }}>
              <span style={{ color:"var(--text-muted)" }}>Monthly Dep.</span>
              <span style={{ fontWeight:600 }}>PKR {monthlyDep.toLocaleString()}</span>
            </div>
          </div>
          <div className="card">
