...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dl-sidebar-logo-img { width: 32px; height: 32px; border-radius: 8px; object-fit: contain; flex-shrink: 0; position: relative; z-index: 1; }
  .dl-sidebar-logo-name { color: white; font-size: 14px; font-weight: 700; line-height: 1.1; position: relative; z-index: 1; }
  .dl-sidebar-logo-sub { color: rgba(255,255,255,0.45); font-size: 9px; position: relative; z-index: 1; }
  ...
  .dl-main { flex: 1; margin-left: 220px; display: flex; flex-direction: column; min-height: 100vh; width: calc(100% - 220px); }
  ...
  .dl-mobile-brand { display: none; align-items: center; gap: 8px; }
  .dl-mobile-brand-logo { width: 24px; height: 24px; border-radius: 6px; object-fit: contain; }
  .dl-mobile-brand-text { font-weight: 700; font-size: 14px; color: #1E3A8A; }
  .dl-mobile-brand-sub { font-size: 9px; color: #94A3B8; }

  @media (max-width: 900px) {
    .dl-sidebar { width: 60px; min-width: 60px; }
    .dl-sidebar-logo-name, .dl-sidebar-logo-sub, .dl-nav-section, .dl-nav-item span:not(.dl-nav-icon), .dl-sidebar-email, .dl-sidebar-signout { display: none; }
    .dl-sidebar-logo { justify-content: center; padding: 14px 0; }
    .dl-nav-item { justify-content: center; padding: 10px; }
    .dl-main { margin-left: 60px; width: calc(100% - 60px); }
  }
  @media (max-width: 640px) {
    .dl-sidebar { transform: translateX(-100%); }
    .dl-sidebar.mobile-open { transform: translateX(0); width: 240px; min-width: 240px; }
    .dl-sidebar.mobile-open .dl-sidebar-logo-name, .dl-sidebar.mobile-open .dl-sidebar-logo-sub, .dl-sidebar.mobile-open .dl-nav-section, .dl-sidebar.mobile-open .dl-nav-item span:not(.dl-nav-icon), .dl-sidebar.mobile-open .dl-sidebar-email, .dl-sidebar.mobile-open .dl-sidebar-signout { display: block; }
    ...
    .dl-main { margin-left: 0; width: 100%; }
    ...
    /* Show mobile brand */
    .dl-mobile-brand { display: flex; margin-right: auto; }
  }
...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const businessName = settings?.business_name || 'OneAccounts'
      ...
      <div className="dl-shell">
        ...
        <aside className="dl-sidebar" id="dl-sidebar">
          <div className="dl-sidebar-logo">
            <img src={logoUrl} alt={businessName} className="dl-sidebar-logo-img" />
            <div>
              <div className="dl-sidebar-logo-name">{businessName}</div>
          ...
          <nav className="dl-sidebar-nav">
            {Object.entries(sections).map(([section, items], secIdx) => (
              <div key={section}>
                ...
                {items.map((item) => (
                  <a key={item.href} href={item.href} className={`dl-nav-item${item.href === '/dashboard' ? ' active' : ''}`}>
        ...
        <div className="dl-main">
          <header className="dl-topbar">
            ...
            <div className="dl-mobile-brand">
              <img src={logoUrl} alt={businessName} className="dl-mobile-brand-logo" />
              <div>
                <div className="dl-mobile-brand-text">{businessName}</div>
                <div className="dl-mobile-brand-sub">by Siqbal</div>
              </div>
            </div>
