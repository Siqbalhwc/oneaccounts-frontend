...
export default function BillsPage() {
  ...
  // 2. Fetch bills (purchase invoices)
  useEffect(() => {
    if (!role) return
    if (!canView) {
      setLoading(false)
  ...
  // Reminder helper (for unpaid bills)
    ...
    <div style={{ padding: 24, background: "var(--bg)", minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: "var(--text)" }}>
      <style>{`
        ...
        @media (max-width: 640px) {
          .header-row, .data-row { grid-template-columns: 100px 70px 150px 80px 60px 80px 45px 45px 45px; column-gap: 4px; padding: 10px 12px; }
        }
      `}</style>
