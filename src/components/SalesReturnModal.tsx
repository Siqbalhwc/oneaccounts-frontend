 ...
 *   create_sales_return_transaction(company, invoice, date, reason)
 ...
 * is blocked (receipts recorded against the invoice, already returned, ...).
 */
...
export default function SalesReturnModal({ invoice, onClose, onDone }: Props) {
  ...
  const [returnDate, setReturnDate] = useState<string>(today < invoice.date ? invoice.date : today)
  ...
  const submit = async () => {
    ...
    if (returnDate < invoice.date) { setError(`Return date cannot be before the invoice date (${invoice.date}).`); return }

    setBusy(true)
    setError("")

    const { data, error: rpcError } = await supabase.rpc("create_sales_return_transaction", {
    ...
    >
      ...
      >
        ...
        <div style={{ background: "var(--bg-soft)", border: "1px solid var(--border)", borderRadius: 10, padding: 12, fontSize: 13, lineHeight: 1.5, marginBottom: 14 }}>
          This fully reverses the invoice in one step:
          <ul style={{ margin: "6px 0 0", paddingLeft: 18 }}>
            <li>the journal entry (all accounts and project / site / activity tags)</li>
            <li>the customer balance and Customer Ledger</li>
            <li>stock quantities (returned at the original cost) and average cost</li>
          </ul>
          <div style={{ marginTop: 6, color: "var(--text-muted)" }}>
            The original invoice is kept and marked <b>Returned</b>. A return document is created and linked to it. If receipts were received against this invoice, reverse them first.
          </div>
        </div>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div>
            ...
            <textarea
              ...
              disabled={busy}
              rows={3}
              placeholder="e.g. wrong customer, goods returned, entered by mistake"
