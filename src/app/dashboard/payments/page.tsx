...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true)
      const { count } = await supabase.from("journal_entries").select("*", { count: "exact", head: true }).like("entry_no", "PAY-%")
      setTotal(count || 0)

      const from = (page - 1) * pageSize
      const to = from + pageSize - 1
      const { data } = await supabase.from("journal_entries")
        .select("id, entry_no, date, description").like("entry_no", "PAY-%").order("date", { ascending: false }).range(from, to)
  ...
  return (
    <div style={{ padding: 24, background: "#EFF4FB", minHeight: "100vh", fontFamily: "Arial" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div><h1 style={{ fontSize: 22, fontWeight: 800, color: "#1E293B", margin: 0 }}>💳 Payments</h1><p style={{ color: "#94A3B8", fontSize: 13, margin: 0 }}>Supplier payment history</p></div>
      ...
      {loading ? <div style={{ textAlign: "center", padding: 40, color: "#94A3B8" }}>Loading...</div> :
        ...
        <div style={{ background: "white", borderRadius: 10, border: "1px solid #E2E8F0", overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "110px 100px 1fr 100px", padding: "10px 16px", background: "#F8FAFC", fontSize: 9, fontWeight: 700, textTransform: "uppercase", color: "#94A3B8" }}>
            <span>Payment No</span><span>Date</span><span>Description</span><span>Amount</span>
          </div>
          {entries.map((e, i) => (
