...
export default function InvestorCapitalPage() {
  ...
  const handleRecordContribution = async () => {
    ...
    try {
      ...
      setContribSaving(false)
      fetchProjectInvestors()
      // Invalidate cached history for this investor so it refetches next expand
  ...
  const toggleHistory = async (pi: any) => {
    ...
    const { data } = await supabase
      ...
      .select("id, amount, contribution_date, reference, status, notes")
      .eq("project_id", selectedProjectId)
...
Write-Host "SUCCESS: investor-capital/page.tsx replaced with contribution history list (Stage 2)." -ForegroundColor Green