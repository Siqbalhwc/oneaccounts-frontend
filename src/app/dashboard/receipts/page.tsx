    ...
    <RoleGuard allowedRoles={["admin", "accountant"]}>
      <div style={{ padding: 24, background: "#0B1120", minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: "#E2E8F0" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: "#F1F5F9" }}>💰 Receipts</h1>
            <p style={{ fontSize: 13, color: "#94A3B8" }}>{canEdit ? "Record customer payments" : "View receipts"}</p>
          </div>
          {canEdit && (
        ...
        <div style={{ maxWidth: 300, marginBottom: 16, position: "relative" }}>
          <input className="input" style={{ paddingLeft: 32, width: "100%" }} placeholder="Search receipt…" value={search} onChange={e => setSearch(e.target.value)} />
        </div>

        {loading ? (
          <div style={{ textAlign: "center", padding: 40, color: "#94A3B8" }}>Loading…</div>
        ) : filtered.length === 0 ? (
          <div className="card" style={{ padding: 40, textAlign: "center", color: "#94A3B8" }}>No receipts found.</div>
        ) : (
          <div className="card" style={{ padding: 0, overflowX: "auto" }}>
            <table>
              ...
              <tbody>
                {filtered.map((rec) => (
                  <tr key={rec.id}>
                    <td style={{ fontWeight: 600, color: "#93C5FD" }}>{rec.receipt_no}</td>
                    <td>{rec.date}</td>
                    <td>{rec.party?.name || "—"}</td>
                    <td style={{ fontWeight: 600 }}>{rec.amount?.toLocaleString()}</td>
                    <td>{rec.payment_method}</td>
                    <td>
                      <button className="btn btn-outline" style={{ padding: 4 }} onClick={() => router.push(`/dashboard/receipts/${rec.id}`)}>
                        <Eye size={14} />
                      </button>
                    </td>
                  </tr>
