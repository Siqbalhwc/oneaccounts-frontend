...
export default function JournalPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase.from("journal_entries").select("*").order("date", { ascending: false }).range(from, to)
  ...
  return (
    <div style={{ padding: 24, background: "#EFF4FB", minHeight: "100vh", fontFamily: "Arial" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div><h1 style={{ fontSize: 22, fontWeight: 800, color: "#1E293B", margin: 0 }}>📓 Journal Entries</h1><p style={{ color: "#94A3B8", fontSize: 13, margin: 0 }}>Double-entry bookkeeping</p></div>
      ...
      {loading ? <div style={{ textAlign: "center", padding: 40 }}>Loading...</div> :
        <div style={{ background: "white", borderRadius: 10, border: "1px solid #E2E8F0", overflow: "hidden" }}>
          {filtered.length === 0 ? <div style={{ padding: 40, textAlign: "center", color: "#94A3B8" }}>No journal entries found</div> :
            filtered.map((e, i) => (
              <div key={e.id}>
                <div onClick={() => toggleExpand(e.id)} style={{ display: "grid", gridTemplateColumns: "110px 1fr 90px 60px", padding: "12px 16px", borderBottom: "1px solid #F1F5F9", fontSize: 13, alignItems: "center", cursor: "pointer", background: expanded === e.id ? "#FAFBFF" : "white" }}>
