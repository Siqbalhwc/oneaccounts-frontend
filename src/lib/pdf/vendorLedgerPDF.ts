...
export interface VendorLedgerPDFData{ companyName:string; companyAddress?:string; companyPhone?:string; companyEmail?:string; companyTagline:string; logoUrl?:string|null; supplierName:string; supplierCode:string; startDate:string; endDate:string; totalDebit:number; totalCredit:number; closingBalance:number; ledgerLines:LedgerLine[] }
...
export async function generateVendorLedgerPDF(data:VendorLedgerPDFData):Promise<jsPDF>{
  ...
  doc.setFont("helvetica","bold").setFontSize(24).setTextColor(...NAVY).text("VENDOR LEDGER",PW-MR,LOGO_Y+8,{align:"right"})
  doc.setFont("helvetica","normal").setFontSize(8.5).setTextColor(...MUTED).text(`${data.supplierCode} – ${data.supplierName}`,PW-MR,LOGO_Y+16,{align:"right"})
